...
import React, { HTMLAttributes, ReactElement, createContext, useContext } from 'react';
...
import { QueryHookOptions, useQuery } from '../hooks/useQuery';
...
describe.each([[true], [false]])('getMarkupFromTree with "suspend: %s"', suspend => {
  ...
  it('should allow network-only fetchPolicy as an option and still render prefetched data', async () => {
    ...
    await expect(
      getMarkupFromTree({
        renderFunction: renderToString,
        tree: <UserDetailsWrapper client={client} suspend={suspend} fetchPolicy="network-only" />
      })
    ).resolves.toMatchInlineSnapshot(`"James"`);
  });
  ...
  it('should pick up queries deep in the render tree', async () => {
    ...
    ).resolves.toMatchInlineSnapshot(`"<div><span>Hi</span><div>James</div></div>"`);
  });
  ...
  it('should handle nested queries that depend on each other', async () => {
    ...
    ).resolves.toMatchInlineSnapshot(`"<div><div>Authorized: <!-- -->true</div>James</div>"`);
  });
  ...
  it('should handle network errors thrown by queries', async () => {
    const client = createMockClient();
    const tree = <UserDetailsWrapper client={client} suspend={suspend} query={NETWORK_ERROR_QUERY} />;

    await expect(getMarkupFromTree({ tree, renderFunction: renderToString })).rejects.toMatchInlineSnapshot(
      `[Error: Network error: Simulating network error]`
    );

    expect(renderToString(tree)).toMatchInlineSnapshot(`"No Current User (failed)"`);
  });

  it('should handle GraphQL errors thrown by queries', async () => {
    const client = createMockClient();
    const tree = <UserDetailsWrapper client={client} suspend={suspend} query={GRAPQHL_ERROR_QUERY} />;
...
`);
  ...
  it("shouldn't run queries if ssr is turned to off", async () => {
    ...
    await expect(
      getMarkupFromTree({
        renderFunction: renderToString,
        tree: <UserDetailsWrapper client={client} ssr={false} suspend={suspend} />
      })
    ).resolves.toMatchInlineSnapshot(`"No Data (skipped)"`);
