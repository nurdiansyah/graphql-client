...
function Task({ onChange, task }: { task: TaskFragment; onChange: (task: TaskFragment) => void }) {
  return (
    <li>
      <input checked={task.completed} onChange={() => onChange(task)} type="checkbox" />
      {task.text}
    </li>
  );
}

function TaskList({ onChange, tasks }: { tasks: TaskFragment[]; onChange: (task: TaskFragment) => void }) {
  return (
    <ul>
      {tasks.map(task => (
...
it('should create a function to perform mutations', done => {
  function TasksWithMutation() {
    ...
    return <TaskList onChange={task => toggleTask({ variables: { taskId: task.id } })} tasks={data.tasks} />;
  }
  ...
  setTimeout(() => {
    const firstCheckbox = container.querySelector<HTMLInputElement>('input:checked')!;
...
it('should allow to pass options forwarded to the mutation', done => {
  function TasksWithMutation() {
    const { data, error, loading } = useQuery(TASKS_QUERY);
    const addTask = useMutation<any, { input: Partial<TaskFragment> }>(ADD_TASK_MUTATION, {
