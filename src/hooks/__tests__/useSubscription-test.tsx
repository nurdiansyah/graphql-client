...
const complitedResults = SAMPLE_TASKS.filter(task => task.completed).map(task => ({
  result: { data: { task } }
}));

const uncomplitedResults = SAMPLE_TASKS.filter(task => !task.completed).map(task => ({
  result: { data: { task } }
}));
...
it('should return the subscription error', done => {
  const link = new MockSubscriptionLink();
  const client = createClient({ link });
...
it('should execute subscription with provided variables', done => {
  ...
  const Component = () => {
    const { data, loading, error } = useSubscription(FILTERED_TASKS_SUBSCRIPTION, { variables });
    if (count === 0) {
      expect(loading).toBe(true);
      expect(error).toBeUndefined();
      expect(data).toBeUndefined();
...
it('should not re-subscription if variables have not changed', done => {
  const variables = { completed: true };
...
it('should re-subscription if variables have changed', done => {
  class MockSubscriptionLinkOverride extends MockSubscriptionLink {
    variables: any;
    request(req: Operation) {
  ...
  const Component = () => {
    ...
    const { data, loading, error } = useSubscription(FILTERED_TASKS_SUBSCRIPTION, {
      variables: { completed }
    });
    if (count === 0) {
      expect(loading).toBe(true);
...
it('should not subscribe if skip option is enabled', async () => {
  ...
  const Component = () => {
    const { data, loading, error } = useSubscription(FILTERED_TASKS_SUBSCRIPTION, { variables, skip: true });
    expect(loading).toBe(true);
    expect(error).toBeUndefined();
    expect(data).toBeUndefined();
    return null;
